...
- 固定速度(规定 FPS): 相对时刻表调度 —— 每帧等待 `现在 + TICK`(帧间隔 ≈ TICK + 帧耗时)
  (已校准: 游戏 time_0/time_1 = **Unix 纳秒**, 10 亿刻度/秒; 默认 `FPS = 1` ->
  TICK=1,000,000,000=0x3B9ACA00, 拆 TICK_HI/TICK_LO 合成 32 位); 相对时刻表防连跳:
  游戏时间跳变/卡顿后最多补走一帧, 不会出现蛇突然闪现一大截;
  想调速度只改 `FPS`(对应的 TICK = 1,000,000,000 / FPS)
- 帧缓冲固定 0x3000(与 DOS 共用屏幕); 数据区 0x2400..0x27FF;
  栈顶 0x7000(避开加载区); 模拟器时间按纳秒推进(1000 单位/指令), CLI 默认 100 万步/秒 = 1 格/秒
