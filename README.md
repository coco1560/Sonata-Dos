...
- 无闪屏: 原版每帧把 64x32 全清(2048 格逐字节写)再重画, 清空瞬间屏幕空白造成闪烁;
  现改为增量重绘 —— 每帧只擦旧蛇尾 + 重绘蛇身, 格子用 store_32 一次写入
- 固定速度(规定 FPS): 相对时刻表调度 —— 每帧等待 `现在 + TICK`(帧间隔 ≈ TICK + 帧耗时)
  (已用 `timecal.asm` 实测: time_0 ≈ 1.263 亿刻度/秒; 默认 `FPS = 1` ->
  TICK=126,277,413=0x0786D725, 拆 TICK_HI/TICK_LO 合成 32 位); 相对时刻表防连跳:
  游戏时间跳变/卡顿后最多补走一帧, 不会出现蛇突然闪现一大截;
  想调速度只改 `FPS`(对应的 TICK = 126,277,413 / FPS)
- 帧缓冲固定 0x3000(与 DOS 共用屏幕); 数据区 0x2400..0x27FF;
  栈顶 0x7000(避开加载区); 模拟器 time_0 按实测 126 单位/指令推进, CLI 默认 100 万步/秒 = 1 格/秒
...
- `tools/test.mjs` — 端到端回归测试(108 项, 全部通过)
...
- 键盘: Enter=10, Backspace=13, Esc=1; 屏幕从 12288 固定读取
